...
def augment_demand(demand_df, peak_start, peak_end):
    aug_demand = [demand_df[hr]*PK_PRICE_EFFECT/1000 if (
        hr >= peak_start and hr <= peak_end) else demand_df[hr]*OFPK_PRICE_EFFECT/1000 for hr in range(0, 24)]
    return aug_demand
...
class PeakPeriodProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        if x[0] == x[1]:
            ...
            out["F"] = np.column_stack(
                [self.cost_fn_income(x), self.cost_fn_peak(x)])
            out["G"] = np.column_stack(
                [self.const_order(x), self.const_period(x)])
        else:
    ...
    def cost_fn_income(self, x):
        # income = calc_income(self)
        # income = 0
        income = calc_income(
            self.demand_df, x[0], x[1], self.peak_cost, self.offpeak_cost)

        return -1 * income

    # cost function = ลดความต้องการไฟฟ้าสูงสุด
    def cost_fn_peak(self, x):
        demands = augment_demand(self.demand_df, x[0], x[1])
        # peak = 0
        peak = max(demands)
        return peak
    ...
    def const_period(self, x):
        return x[1] - x[0] - self.max_period
        # return 0
...
st.write(
    f'ค่าไฟช่วง peak {peak_cost} บาท/หน่วย และ off-peak {offpeak_cost} บาท/หน่วย')
income = calc_income(df.WORKDAY, peak_start, peak_end, peak_cost, offpeak_cost)
st.write(
    f'ค่าไฟฟ้ารวม: {income:.2f} พันล้านบาท และความต้องการไฟฟ้าสูงสุด {max(df.WORKDAY)/1000:.2f} MW')

# เงื่อนไข
st.subheader('เงื่อนไข')
max_period = st.number_input('ช่วง peak ไม่เกินกี่ชั่วโมงต่อวัน',
                             min_value=1, max_value=24, value=8, step=1, key='max_period')
...
if solve_flag:
    ...
    sol = res.X[0]
    out = res.F[0]
    st.slider('Peak period', min_value=0, max_value=23, value=[
              sol[0], sol[1]], step=1, key='peak_period')
    st.write(
        f'ค่าไฟฟ้ารวม: {-out[0]:.2f} พันล้านบาท และความต้องการไฟฟ้าสูงสุด {out[1]:.2f} MW')
